...
const TopStrip = () => {
  ...
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
      const optionsDate = { day: "2-digit" as const, month: "short" as const, year: "numeric" as const };
  ...
  return (
    <div className="w-full flex items-center justify-between px-8 md:px-12 bg-[#1a2b5f] text-white h-[32px] text-xs font-sans">
        ...
        <div className="flex items-center gap-2">
          <Show when="signed-out">
            <SignInButton mode="modal">
              <button className="bg-[#f5a623] hover:bg-[#e69b20] transition-colors text-[#1a2b5f] px-2 py-0.5 font-bold rounded-sm inline-block">
                {t('topStrip.signIn')}
              </button>
            </SignInButton>
          </Show>
          <Show when="signed-in">
            <div className="flex items-center h-full pt-1">
              <UserButton appearance={{ elements: { userButtonAvatarBox: "w-[22px] h-[22px]" } }} />
            </div>
          </Show>
