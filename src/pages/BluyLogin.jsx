...
const BluyLogin = () => {
  ...
  const handleResend = () => {
    setResendMsg('OTP has been sent again.');
    setOtp('');
  ...
  const panelLinks = ['Home', 'Guidelines', 'Partner Network', 'Project Catalog', 'Support', 'Help Docs'];
    ...
    <div style={{ minHeight: '100vh', background: '#f0f2f5', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 16px' }}>
      <div style={{ display: 'flex', width: '100%', maxWidth: 940, borderRadius: 16, overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.15)' }}>
        ...
        <div style={{ background: '#fff', padding: '52px 56px', width: '60%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <h2 style={{ fontSize: '1.55rem', fontWeight: 700, color: '#3600a0', marginBottom: 40, textAlign: 'center' }}>Sign in to your account</h2>
          ...
          <form onSubmit={otpSent ? handleVerify : handleGetOtp} style={{ width: '100%', maxWidth: 420 }}>
            <div style={{ marginBottom: 28 }}>
              <label style={{ display: 'block', fontSize: '0.85rem', color: '#3600a0', fontWeight: 600, marginBottom: 6 }}>Aadhaar Number</label>
              <input type="text" inputMode="numeric" placeholder="1261 8891 7711" value={aadhaar} onChange={(e) => setAadhaar(formatAadhaar(e.target.value))} disabled={otpSent} style={{ width: '100%', border: '1.5px solid #c4b5fd', borderRadius: 8, padding: '12px 14px', fontSize: '1rem', outline: 'none', boxSizing: 'border-box', letterSpacing: '0.08em', background: otpSent ? '#f8f8ff' : '#fff' }} />
            </div>

            {otpSent && (
              <div style={{ marginBottom: 28 }}>
                <label style={{ display: 'block', fontSize: '0.85rem', color: '#3600a0', fontWeight: 600, marginBottom: 6 }}>Enter OTP</label>
                <input type="text" inputMode="numeric" placeholder="- - - - - -" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))} style={{ width: '100%', border: '1.5px solid #c4b5fd', borderRadius: 8, padding: '12px 14px', fontSize: '1.1rem', outline: 'none', boxSizing: 'border-box', letterSpacing: '0.2em' }} autoFocus />
              </div>
            )}
            ...
            <div style={{ textAlign: 'center' }}>
              <a href={BLUY_2024} target="_blank" rel="noopener noreferrer" style={{ color: '#3600a0', fontSize: '0.85rem', textDecoration: 'underline', lineHeight: 1.5 }}>
                Login to Business Launch Hub 2024-25
              </a>
            </div>
          </form>
        </div>
